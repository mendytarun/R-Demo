...
colnames(mov2)[which(names(mov2) == "Adjusted.Gross...mill.")] <- "Adj.Gross"

colnames(mov2)[c(8,9,12,13,14,15,16,17,18)] <- c("Budget","Gross","OverseasGross","Overseas.Percent","Profit","Profit.Percent","Runtime(min)",
                                          "USGross","USGross.Percent")
str(mov2)
...
p <- ggplot(data=mov2, aes(x=Genre, y=USGross.Percent))
